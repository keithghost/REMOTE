...
module.exports = {
    ...
    onStart: async function ({ msg, bot, match, event }) {
        try {
            // Get the correct path to commands directory (same level as index.js)
            const commandDir = path.join(__dirname, 'remote_commands_cache');
            
            // Check if directory exists
            if (!fs.existsSync(commandDir)) {
                throw new Error(`Commands directory not found at: ${commandDir}`);
            }
