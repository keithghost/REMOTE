...
async (from, client, conText) => {
  ...
  try {
    ...
    if (q.match(/(youtube\.com|youtu\.be)/i)) {
      ...
      const searchResponse = await axios.get(`https://apiskeith.top/search/yts?query=${encodeURIComponent(q)}`);
    ...
    const downloadResponse = await axios.get(`https://apiskeith.top/download/audio?url=${encodeURIComponent(videoUrl)}`);
...
async (from, client, conText) => {
  ...
  try {
    ...
    const downloadResponse = await axios.get(`https://apiskeith.top/download/video?url=${encodeURIComponent(videoUrl)}`);
    const downloadUrl = downloadResponse.data?.result;
    if (!downloadUrl) return;
