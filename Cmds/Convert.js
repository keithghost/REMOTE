...
keith({
  ...
  try {
    // Call your API
    const apiUrl = `https://apiskeith.top/shortener/bitly?url=${encodeURIComponent(q)}`;
...
async (from, client, conText) => {
  ...
  try {
    const apiUrl = `https://apiskeith.top/shortener/tinube?url=${encodeURIComponent(url)}&name=${encodeURIComponent(name)}`;
...
async (from, client, conText) => {
  ...
  try {
    const apiUrl = `https://apiskeith.top/shortener/tinyurl?url=${encodeURIComponent(q)}`;
    const response = await axios.get(apiUrl, { timeout: 100000 });
