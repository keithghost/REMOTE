...
async (from, client, conText) => {
  ...
  try {
    const response = await axios.get("https://apiskeith.top/dl/hentaivid", { timeout: 100000 });
...
async (from, client, conText) => {
  ...
  try {
    ...
    if (!/^https?:\/\/(www\.)?xvideos\.com\//i.test(q)) {
      const searchRes = await axios.get(`https://apiskeith.top/search/searchxvideos?q=${encodeURIComponent(q)}`);
      const topResult = searchRes.data?.result?.find(x => x.url?.includes("videos.com"));
      if (!topResult?.url) return;
      videoUrl = topResult.url;
    }

    const dlRes = await axios.get(`https://apiskeith.top/download/xvideos?url=${encodeURIComponent(videoUrl)}`);
...
async (from, client, conText) => {
  ...
  try {
    const apiUrl = `https://apiskeith.top/download/spotify?q=${encodeURIComponent(q)}`;
    const response = await axios.get(apiUrl, { timeout: 100000 });
...
async (from, client, conText) => {
  ...
  try {
    const apiUrl = `https://apiskeith.top/download/mfire?url=${encodeURIComponent(q)}`;
    const response = await axios.get(apiUrl, { timeout: 100000 });
    const result = response.data?.result;
...
async (from, client, conText) => {
  ...
  try {
    ...
    if (!/^https?:\/\/(m\.)?soundcloud\.com\//i.test(q)) {
      const searchRes = await axios.get(`https://apiskeith.top/search/soundcloud?q=${encodeURIComponent(q)}`);
    ...
    const dlRes = await axios.get(`https://apiskeith.top/download/soundcloud?url=${encodeURIComponent(trackUrl)}`);
