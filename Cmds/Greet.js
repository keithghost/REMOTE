...
const transformText = (text, fontType) => {
    const fontMap = fontMaps[fontType];
    let result = '';
...
const toFancyUppercaseFont = (text) => transformText(text, 'uppercase');
const toFancyLowercaseFont = (text) => transformText(text, 'lowercase');
...
function getCategoryCommands(categoryGroups, selectedNumber) {
    ...
    const commandsText = commandsInCategory.length > 0
    ...
    return { text: commandsText, category: selectedCategory };
}
...
keith({
    ...
    react: "📜",
    filename: __filename
}, async ({ client, m, prefix, url, author, botname, pushname }) => {
    try {
        ...
        const authorName = pushname || 'User';
        const contactNumber = m.sender?.split('@')[0] || 'unknown';
        ...
        // Main menu message
        const menuMessage = `
*╰► ${toFancyUppercaseFont(greeting)} ${authorName}!*
